from download_and_checks_image import download_image, checks_format
...
def download_img_apod(count: int, api_key: str = DEMO_KEY) -> None :
    ...
    for num, link in enumerate(response.json()):
        if checks_format(link['url']) != JPG_FORMAT:
            continue
        download_image(link['url'], f'{NASA_PATH}{num}{JPG_FORMAT}')


def download_img_epic(count: int, api_key: str = DEMO_KEY) -> None:
    ...
    for num_dict, value_dict in enumerate(response.json()):
        epic_images[num_dict] = {
            'date': value_dict['date'][:10],
    ...
    for num in range(count):
        ...
        image_url = f'{EPIC_ARCHIVE}/{year}/{month}/{day}/png/{image_id}{PNG_FORMAT}?api_key={api_key}'

        download_image(image_url, f'{EPIC_PATH}{num}{PNG_FORMAT}')
